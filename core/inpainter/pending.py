...
from inpainter.session import ensure_auth_dir, now_secs
...
def take_pending(state: str) -> PendingSignIn:
    ...
    try:
        ...
        raise CoreError("no sign-in is in progress") from exc
    except json.JSONDecodeError as exc:
        _remove(path)
        raise CoreError("pending sign-in is unreadable") from exc
    ...
    try:
        ...
        _remove(path)
        raise CoreError("pending sign-in is invalid") from exc

    if pending.expires_at <= now_secs():
        _remove(path)
        raise CoreError("pending sign-in expired")
    if pending.state != state:
        raise CoreError("sign-in state mismatch")
    _remove(path)
    return pending
