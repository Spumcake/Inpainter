...
- `providers/openai`: `uv run python -m app.main` (port 3102).
...
apps/cli/client/.venv/bin/python -m unittest discover -s core/tests -v
PYTHONPATH=providers/openai apps/cli/client/.venv/bin/python -m unittest discover -s providers/openai/tests -v
```
