...
def refresh_session(refresh_token: str) -> dict:
    ...
    try:
        ...
        raise CliError(f"refresh request failed: {exc}") from exc

    try:
        body = response.json()
    except ValueError as exc:
        raise CliError("refresh response was not json") from exc

    if not response.is_success:
        raise CliError("refresh failed")
    if not isinstance(body, dict):
        raise CliError("refresh response was not json")
    return body
