...
def load_session() -> StoredSession | None:
    ...
    except (KeyError, TypeError, ValueError, json.JSONDecodeError, CliError):
        return None
...
def session_from_token_body(body: dict) -> StoredSession:
    ...
    if not isinstance(access_token, str) or not access_token:
        raise CliError("missing access_token")
    if not isinstance(refresh_token, str) or not refresh_token:
        raise CliError("missing refresh_token")
...
def _load_or_create_key(directory: Path) -> bytes:
    ...
    if path.exists():
        key = path.read_bytes()
        if len(key) != 32:
            raise CliError("session key has unexpected length")
        return key
    key = os.urandom(32)
...
def _decrypt(key: bytes, blob: bytes) -> bytes:
    if len(blob) < 13:
        raise CliError("session file is truncated")
    try:
        return AESGCM(key).decrypt(blob[:12], blob[12:], None)
    except Exception as exc:
        raise CliError("failed to decrypt session") from exc
